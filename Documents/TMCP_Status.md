...
- **Retirements reflected in the count:**
  - **AI Navigation** — retired Session 17, 2026-06-04; superseded by Ivan-Murzak's official `navigation-*` 10-tool Extension, 4 `nav-*` tools removed.
  - **Cinemachine** — retired Session 16, 2026-06-04; superseded by the official `cinemachine-*` Extension, 5 `cm-*` tools removed.
  - **Asset Inventory** — removed back in Session 6 but lingered as a stale table row + orphaned `HAS_ASSETINVENTORY` define until the 2026-06-04 audit dropped both.
...
1. **Asmdef with direct refs** -- asset has its own assembly (e.g., AnimationRigging, Animancer, Feel, Terrain25D, BridgeBuilder25D, BehaviorDesigner). Asmdef references asset assembly + MCP assemblies, uses `defineConstraints`.
2. **Asmdef with reflection** -- asset is in Assembly-CSharp (no asmdef), **or its owning assembly varies with how the user installed it**. Asmdef references only MCP assemblies, all asset type access via reflection (e.g., DecalCollider, TextureStudio, MasterAudio, DialogueSystem, RopeToolkit, FinalIK, PWB, QuestForge, PMG, Chunity, **DOTween**). Choose this whenever the asset can land in `Assembly-CSharp` under *any* supported layout -- an asmdef physically cannot reference a predefined assembly, so a direct ref breaks for exactly those users (see the DOTween Pro row above).
...
| Asmdef pattern | Every tool group has its own asmdef (as of 2026-08-02) using `overrideReferences: true` + `ReflectorNet.dll`, and keeps its `#if HAS_*` guard as belt-and-suspenders. Historically, groups without an asmdef lived in `Assembly-CSharp-Editor` and relied on the `#if` guard alone. |
...
| Stale `HAS_*` defines after UPM-style asset removal | **Logged TVD11 (2026-05-23).** `MCPToolsAssetPostprocessor.OnPostprocessAllAssets` does NOT reliably catch UPM Package Manager removals — its `path.EndsWith(".asmdef"\|".cs"\|".dll")` heuristic misses the deleted paths and the stale `HAS_X` define stays in `PlayerSettings`. On next compile, the integration's `defineConstraints`-gated asmdef compiles against a now-missing namespace and the project breaks with `CS0246`. `MCPToolsDefineManager` `[InitializeOnLoad]` cannot recover, because the compile-error cascade prevents `MCPTools.Editor` itself from loading (despite it having no `defineConstraints` of its own) — so **Tools > TecVooDoo > Rescan MCP Defines** is unreachable from this state. Recovery procedure: see **Note 1** below the table. |
| ~~11 integration folders without asmdefs leak into Assembly-CSharp-Editor~~ — **RESOLVED 2026-08-02** | **Logged TVD11 (2026-05-23); closed 2026-08-02.** The folders were BehaviorDesigner, Feel, DOTween, Terrain25D, BridgeBuilder25D, JuicyActions, BoingKit, MudBun, Lumen, uLipSync, Timeflow. With no asmdef, `#if HAS_*` was the only guard — `defineConstraints` cannot intervene when there is no asmdef to set them on — so a stale `HAS_X` (prior row) broke the whole project's Editor assembly rather than one tool group. 10 were asmdef-ified in Session 24 (`ed1267b`); **DOTween was reverted (`a2804f0`) and stayed the lone holdout** until the reflection refactor (2026-08-02) let it carry `MCPTools.DOTween.Editor` too. **Every tool group now has its own asmdef; grep-verified 0 remaining.** |

**Note 1 — recovering from a stale `HAS_*` cascade.** Inline-mirror the `Entries` table + `FindType()` from [MCPToolsDefineManager.cs](../Editor/MCPToolsDefineManager.cs) inside a `script-execute` call. Roslyn dynamic compile runs *outside* the project's asmdef graph, so it can write `PlayerSettings.SetScriptingDefineSymbols` even while `MCPTools.Editor` is offline. Historically this was worst-case for the asmdef-less integrations, whose `#if HAS_*` code leaked into `Assembly-CSharp-Editor` and took the whole Editor assembly down with them; with every group now asmdef-isolated, a stale define is confined to its own tool group.
...
- **Sites:** `Editor/MCPToolsDefineManager.cs` (127,56) + (139,56) -- the two `FindType` passes now share one `IReadOnlyList<Assembly>` fetched once; `DOTween/Editor/Tool_DOTween.cs` (38,37) -- added `using System.Collections.Generic;`.
- **API:** `UnityEngine.Assemblies.CurrentAssemblies.GetLoadedAssemblies()` (public, not obsolete, `UnityEngine.CoreModule`; confirmed by reflection on 6000.6.2f1). Guarded `#if UNITY_6000_4_OR_NEWER` with `AppDomain` fallback, so the `6000.0` floor is untouched. **The 6.4 cutoff comes from a third-party release note**, not Unity docs -- if a 6000.4/6000.5 project throws `CS0234` on `UnityEngine.Assemblies`, raise the guard.
- **Verified:** `CleanBuildCache` rebuild, all 129 TVD assemblies; control warnings (TVU `UAC0009`, CodeStage `UAC0005`) fired while ours did not. `DOTweenAnimation` + nested `AnimationType` resolve; `FindType` works on both the exact-assembly and wrong-assembly passes; 17 tools register on MCP 0.93.0.
- **Not included:** `BehaviorDesigner` asmdef + `Tool_RayFire.Rigid.cs` working-tree edits present at the time belong to another session -- deliberately not staged.

### Gate hardening -- the 13 MCP-namespace groups now carry `UNITY_MCP_READY` (August 27, 2026) (TVD Session 43)

**What changed.** 13 tool-group asmdefs gained `"UNITY_MCP_READY"` in `defineConstraints`, alongside their existing `HAS_*`: **ALINE, AnimationRigging, DamageNumbersPro, FinalIK, Flexalon, HeathenBallistics, HeathenPhysics, MagicaCloth2, MalbersAC, PrefabWorldBuilder, RayFire, RetargetPro, UCC.** No `.cs` file touched.

**The defect it addresses.** MCP's `RecompileGate.Reset()` strips `UNITY_MCP_READY` from every build target during a version crossing, which excludes `com.IvanMurzak.Unity.MCP.Editor` / `.Runtime` from compilation entirely. **MCP's own Extension packages gate their asmdefs on that same define, so they simply drop out of the build. TMCP's equivalents did not** -- they gated only on their `HAS_*`, so they stayed in the build and **failed** against a namespace that no longer existed, throwing `CS0234: 'Unity' does not exist in the namespace 'com.IvanMurzak'`. That is not cosmetic: `EnsureReadyDefine()` re-adds the define **only after a successful restore**, so a project left red by TMCP can never close its own gate -- the deadlock the "dance" (remove TMCP, bump, re-add) exists to break by hand. **This makes the package do it itself.**

**Which 13, and why it is exactly these.** The vulnerable set is every group importing the **gated** `com.IvanMurzak.Unity.MCP.*` namespaces (`.Editor.Utils`, `.Runtime.Data`, `.Runtime.Extensions`). Groups importing only `com.IvanMurzak.McpPlugin` / `com.IvanMurzak.ReflectorNet` are **immune** -- those types live in the NuGet DLLs under `Assets/Plugins/NuGet/`, which no define gates. **Derive the list, never recall it:** `grep -rn "^using com\.IvanMurzak\.Unity\.MCP" . --include="*.cs"`. The long-standing fleet belief that "it's always ALINE and RetargetPro" was a projection of which assets each project installs -- TVD activates 1 of the 13, Sandbox 2, VNPC hit it via AnimationRigging.

**Deliberately NOT added: `UNITY_MCP_DEPS_*`.** MCP 0.89.0 introduced a second gate define `UNITY_MCP_DEPS_2` and 0.90.0 **renamed it to `UNITY_MCP_DEPS_3`**. A constraint naming a specific suffix would silently gate the group **off permanently** at the next rename -- the assembly compiles green and registers **zero** tools, which is the failure mode that is hardest to notice. `UNITY_MCP_READY` has been stable across every version to date and is what the vendor's own Extensions use.

**Scope -- what this does NOT fix.** Only the **strip** window. It does **not** help across a define **rename**, where `UNITY_MCP_READY` is present (so constrained assemblies still build) while the new core is gated out behind `DEPS_<n>`. TVD S43 measured 1,305 errors from exactly that window, in MCP's own Extensions, which already carry the `READY` constraint.

**Verification.** `MCPToolsAsmdefSync` rewrites `precompiledReferences` only, never `defineConstraints`, so the change survives domain reloads. All 13 files re-parsed as valid JSON. Post-change in TVD (MCP 0.90.0, gate closed): `assets-refresh` -> `isCompiling=False`, `scriptCompilationFailed=False`, 124 editor assemblies, `MCPTools.RetargetPro.Editor` loaded, **all 17 TMCP tools still registered** across dotween/maintainer/retarget/tcc. **Was inert while the define is set -- validated on two live crossings the next day, see below.** Note the asmdefs are not uniformly formatted: 12 use a multi-line `defineConstraints` array, **FinalIK uses a single-line one**, which is why a naive multi-line edit silently skips it.

**VALIDATED next day (August 28, 2026) on two live crossings -- both the promise and the stated limit held.** The entry above said the change was inert until the next crossing and untested there. It was tested within hours, on two projects Rune bumped out of session, analysed from the TVD side:

| | TVD (unhardened, Aug 27) | FearSteez | BloodMiner |
|---|---|---|---|
| Crossing | 0.89.0 -> 0.90.0 | 0.87.0 -> 0.89.0 -> 0.90.0 | **0.89.0 -> 0.90.0** |
| Vulnerable groups active | 1 | **5** | 3 |
| 2nd gate cycle opened by | `ForceResolve()` | -- | **`ForceResolve()`** |
| Errors in STRIP windows | **24** | **0** (3 windows) | **0** (2 windows) |
| Errors in RENAME window | 1,305 | 1,413 (114 TMCP) | 1,311 (12 TMCP) |
| Dance | **required** | none | none |

**BloodMiner is near-controlled: the same crossing TVD deadlocked on, the same operator action** (both second gate cycles opened by `NuGetResolverMenu.ForceResolve()`, not the Update button), **3x the vulnerable-group exposure, and its gate closed unaided** at `Editor.log:32652` -- where TVD's refused to close across two completed `ForceResolve` restores and only closed once TMCP was removed by hand.

**The strip-window zeroes are exclusion, not idleness.** Compilation genuinely ran inside every strip window on both projects (FearSteez 3/4/3 assembly-reload events across its three, BM 3 and 2 across its two) and produced no errors, because the hardened asmdefs were excluded by the missing `UNITY_MCP_READY` exactly as MCP's own Extensions are. **No group was stranded** -- all 13 `MCPTools.*.Editor` assemblies reload on FearSteez and all 8 on BM after their final closes, including every active vulnerable group.

**The documented limit held, which matters as much as the promise.** Every TMCP error on both projects fell in the **rename** window -- FearSteez 114 (Flexalon 78, FinalIK 24, RetargetPro 6, DamageNumbersPro 3, ALINE 3), BM 12 (RetargetPro 6, DamageNumbersPro 3, ALINE 3) -- where `UNITY_MCP_READY` is present so these groups still compile while the new core sits behind `UNITY_MCP_DEPS_3`. **A fix for that window would have to track the renaming suffix, which is exactly what this change refuses to do.**

**Evidence grade, stated honestly.** Log- and assembly-load-grade, gathered from outside each project. **Assembly-load is compile-grade, not registration-grade** -- only `unity-tool-list` run from each project's own session rules out the "compiles green, registers zero" mode. Those confirmations were in progress from the FearSteez and BloodMiner sessions when this was written.
...
Closes the arc opened by the June 25 revert below. **DOTween was the last tool group in TMCP without an asmdef** (57 groups, 56 had one); it now has `MCPTools.DOTween.Editor`, and **grep confirms 0 groups remain asmdef-less**.

**The problem, restated from `a2804f0`.** DOTween Pro ships `DOTweenAnimation.cs` as loose *source*, so the assembly that owns `DG.Tweening.DOTweenAnimation` depends on whether the user ran DOTween's "Create ASMDEF" utility:

| DOTween layout | `DOTweenAnimation` compiles into | Old asmdef (`ed1267b`) |
|---|---|---|
| Create ASMDEF **on** (TVD) | `DOTweenPro.Scripts` | worked -- it referenced that assembly |
| Create ASMDEF **off** (SetDesign) | `Assembly-CSharp` | **`CS0246`** -- an asmdef cannot reference a predefined assembly |

**The fix.** `DOTweenAnimation` is now resolved by type name at runtime (`FindType` scans `AppDomain.CurrentDomain.GetAssemblies()`), so the asmdef binds *nothing* that varies by layout. Diff against the reverted asmdef is exactly the removal of the layout-dependent bindings -- `references` lost `DOTweenPro.Scripts` and `DOTween.Modules`; `precompiledReferences` lost `DOTweenPro.dll`. What remains is `DOTween.dll` (DOTween **core**, which ships as a precompiled DLL in *both* layouts, so `DOTween` / `Ease` / `LoopType` / `Tween` stay strongly typed). `Tool_DOTween.Global.cs` needed no change at all -- it only touches core.

**Verification (TVD, live).** Compiled clean (`scriptCompilationFailed=False`); assembly count 396 -> **397**, the new one; all **4 `dotween-*` tools registered**; every tool exercised end-to-end -- add (Move/Fade/Color), query (all three end-value branches), all 6 play actions, global. The asmdef survived the auto-sync domain reload byte-identical, because `MCPToolsAsmdefSync.UpdateAsmdef` preserves non-managed `precompiledReferences` entries and the ordering already matches its output.

**Layout coverage, scoped honestly.** The "Create ASMDEF on" layout is verified live in TVD. The "off" layout is verified **structurally, not by running SetDesign**: the compiled assembly's referenced-assembly list is `mscorlib, System, McpPlugin, UnityEngine.CoreModule, DOTween, ReflectorNet, UnityEditor.CoreModule` -- it contains no `DOTweenPro*` and no `Assembly-CSharp`, so the `CS0246` failure mode has no binding left to fail on, and `FindType`'s assembly scan reaches `Assembly-CSharp` at runtime. Residual assumption: a project that builds DOTween **core** from source (no `DOTween.dll`) is still untested; no such layout is in evidence across the fleet.

**Traps caught while writing it.** (1) `DOTweenAnimation` overloads `DORestart()` / `DORestart(bool)`, so a name-only `GetMethod` throws `AmbiguousMatchException` -- all parameterless calls resolve via an explicit `Type.EmptyTypes` signature. (2) Every member the tools touch is a public **field**, not a property (`DOTweenAnimation` exposes no public instance properties); `tween` is inherited from `ABSAnimationComponent` and is found by normal inherited-public lookup. (3) `System.ComponentModel` (imported for `[Description]`) also defines `Component`, so the three tool files carry a `using Component = UnityEngine.Component;` alias -- the old code never hit this because it named the concrete type.

**Incidental fix:** the invalid-`animationType` error now lists the enum values read from the **installed** DOTween Pro (`Enum.GetNames`) instead of a hardcoded list that was missing six of them (`None`, `CameraAspect`, `CameraBackgroundColor`, `CameraOrthoSize`, `CameraPixelRect`, `CameraRect`). Invalid `action` on `dotween-play` is now also rejected up front rather than only when the id filter happened to match.

**Doc sweep in the same pass:** the 3 remaining >1,000-char lines are gone (2 prose blocks restructured into bullets; the stale-`HAS_*` gotcha row shortened by extracting its recovery procedure into *Note 1* under the table, since a Markdown table row cannot wrap). Live sections corrected for the change: the DOTween Pro tool-table row, the compilation-pattern list (DOTween moves from "direct refs" to "reflection"), the asmdef-pattern gotcha row, and the leaky-folder row marked RESOLVED. Dated Session Log entries left as written.
...
**Gotcha hit this session — `MonoScript.GetClass()` returns null when files were added to a package source folder before the asmdef.**

- **Symptom:** file on disk with `.meta` and the right partial-class declaration, Unity recognizes it as a `MonoScript` via `assets-find`, but `CompilationPipeline.GetAssemblies(...).First(a.name=="MCPTools.UMotionPro.Editor").sourceFiles` only lists *some* of the files in the folder (4 of 5 in my case — `Tool_UMotionPro.Export.cs` was missing while `Tool_UMotionPro.Project.cs` was present).
- **Did NOT recover it:** `AssetDatabase.Refresh(ForceUpdate)` + `CompilationPipeline.RequestScriptCompilation(CleanBuildCache)`; nor deleting the `.meta` and re-refreshing (Unity restored the meta from cache).
- **Fix that worked:** rename the file (e.g. `Tool_UMotionPro.Export.cs` → `Tool_UMotionPro.ExportV2.cs`). The filename change forced Unity to treat it as a brand-new asset and add it to the asmdef's source list.
- **Timing:** ~4.5 minutes for the rebuild to flush through, then McpPlugin's `SkillFileGenerator` re-runs for another ~30s — the "running backend" lag the user sees after the compile-progress bar finishes is the skill regeneration, not the compile itself.
- **General rule:** if a fresh tool file refuses to compile into its asmdef even though the file/.meta/class shape look right, rename it.
